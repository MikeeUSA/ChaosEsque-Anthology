...
CLASS(XonoticMapInfoDialog) EXTENDS(XonoticDialog)
	...
	ATTRIB(XonoticMapInfoDialog, rows, float, 14)
...
void XonoticMapInfoDialog_loadMapInfo(entity me, float i, entity mlb)
{
	...
	{	
		//Try again, this time using the quake3 standard
		me.currentMapPreviewImage = strzone(strcat("/levelshots/", MapInfo_Map_bspname));
		if(draw_PictureSize(me.currentMapPreviewImage) == '0 0 0')
...
void XonoticMapInfoDialog_fill(entity me)
{
	...
	me.TR(me);
		me.TDempty(me, 0.2);
		me.TD(me, me.rows - 4, 3, e = makeXonoticImage(string_null, 4.0/3.0));
		me.previewImage = e;
	...
	n = ceil(GameType_GetCount() / (me.rows - 8));
	wgt = (w - 0.2) / n;
	for(i = 0; i < GameType_GetCount(); ++i)
	{
		if(mod(i, n) == 0)
