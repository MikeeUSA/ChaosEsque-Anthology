...
void XonoticMapList_refilter(entity me)
{
	...
	{
		if (!(cvar("menu_levelshotnodraw"))) {
		if (!(cvar("menu_levelshotnopreache"))) {
		if (draw_PreloadPicture(strcat("/maps/", MapInfo_BSPName_ByID(i))) == "")
		{
			if (cvar("menu_levelshot_q3")) {
				//If we fail to find a xonotic levelshot, check for a quake3 levelshot
				//NOTE: This increses the numcachepics even if it DOESN'T Exist
				//(Same for above /maps/ levelshot)
