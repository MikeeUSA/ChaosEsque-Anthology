...
{
	float sortbuf, glob, i, risque;
	string fn;

	risque = cvar("cl_risque");
	...
	for(i = 0; i < search_getsize(glob); ++i)
	{
		...
		if(!get_model_parameters(fn, -1))
			continue;
		if(get_model_parameters_risque > risque)
			continue;
...
void XonoticPlayerModelSelector_saveCvars(entity me)
{
	...
	if (me.currentModelSpecies == SPECIES_HUMAN) {
		...
		cvar_set("hud_damage_color", "0.75 0 0");	
	} else if  (me.currentModelSpecies == SPECIES_ROBOT_RUSTY) {
		cvar_set("hud_damage_color", "0.2 0.1 0.4");
	} else if  (me.currentModelSpecies == SPECIES_ROBOT_SHINY) {
		cvar_set("hud_damage_color", "0.2 0.1 0.4");
	} else if  (me.currentModelSpecies == SPECIES_INSECT) {
		cvar_set("hud_damage_color", "1 1 0");
	} else if  (me.currentModelSpecies == SPECIES_SKELETON) {
		cvar_set("hud_damage_color", "1 1 1");
	} else if  (me.currentModelSpecies == SPECIES_STONE) {
		cvar_set("hud_damage_color", "0.5 0.5 0.5");
	} else if  (me.currentModelSpecies == SPECIES_TAR) {
		cvar_set("hud_damage_color", "0.01 0.01 0.01");
	} else if  (me.currentModelSpecies == SPECIES_FAE) {
		cvar_set("hud_damage_color", "1 0 0");
